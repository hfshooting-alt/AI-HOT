...
| 功能 | 入口/实现 | 主要输入 → 输出 |
| --- | --- | --- |
| Manus 发现与正文 | `manus_source/runner.py`、`content_phase.py` | `config/manus_sources.json`、`prompts/` → `work/manus/` |
...
| 快照与日报/周报 | `build_snapshot.py` | AIHOT API、Manus feed、`archive/` → `public/` |
| 融资公司表 | `funding_table.py`、`funding/` | 快照/feed → `data/funding/`、`public/funding-table.json` |
| 人工标注与评测 | `make_annotation_sheet.py`、`eval_tagging.py` | 归档与标注集 → `eval/` 与指标 |
| 静态快照提取工具 | `extract_snapshot.mjs` | 旧静态 HTML → JSON |
| 本地设置服务 | `settings-server.mjs` | 设置页请求 → 根目录 `.env` |
| 前端开发与构建 | `vinext.mjs` | 跨平台调用 vinext，构建输出 `dist/` |
