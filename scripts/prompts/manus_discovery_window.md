...
Official Jiqizhixin 使用配置的机器之心「产业报道」列表。列表包含多个发布者，作者缺失不阻断收录，来源按配置入口和同站文章URL核实；ScienceAI、新闻资讯及其他机构文章不归入机器之心。打开详情核实文章内容和完整发布时间，署名可为空；保留官网文章 URL，不能标成微信公众号原文。列表若不能加载、详情身份不明或分页边界无法核实，按失败规则记录，不推定窗口无文章。

机器之心详情正文由浏览器渲染。若文本抓取只有导航或登录入口，须用浏览器读取渲染后的文章区域，确认文章标题和正文；有署名则记录，无署名则author=null；不能仅因页面有“登录”就判定正文不可访问。详情仅显示相对时间时，可使用同URL的列表卡片上明确的绝对发布时间，note分别记录署名和时间的证据页；author仅填写实际看到的署名，缺失填null。此步骤仍受原有等待、重开及费用上限约束，真实登录墙或验证拦截不得绕过。
