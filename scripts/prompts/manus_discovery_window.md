...
机器之心详情正文由浏览器渲染。若文本抓取只有导航或登录入口，须用浏览器读取渲染后的文章区域，确认文章标题、正文和“机器之心 原创”署名；不能仅因页面有“登录”就判定正文不可访问。详情仅显示相对时间时，可使用同URL的列表卡片上明确的绝对发布时间，note分别记录署名和时间的证据页；author填写已核实的媒体署名。此步骤仍受原有等待、重开及费用上限约束，真实登录墙或验证拦截不得绕过。
