...
- Official Jiqizhixin 使用配置的「产业报道」列表，来源按配置入口和同站文章URL核实，作者缺失允许author=null，不阻断收录。列表中的 ScienceAI、新闻资讯及其他机构文章不能归入机器之心；逐篇核实详情署名、日期与时间，保留官网文章 URL，不标作微信公众号原文。加载失败或无法确认覆盖范围时标记 failed。
