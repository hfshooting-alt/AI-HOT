...
class DailyResearchTest(unittest.TestCase):
    ...
    def test_isolated_failure_provisional_value_and_no_repeat(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            result=enrich(original,{},d,read_fn=page,propose_fn=model,rules={})
            ...
            repeat=enrich(unchanged,{},d,read_fn=Mock(side_effect=ValueError),propose_fn=model,rules={})
    ...
    def test_budget_caps_requests_and_keeps_remainder(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            result=enrich({'companies':[sample(str(i)) for i in range(8)]},{},d,read_fn=page,propose_fn=model,rules={})
            self.assertEqual(model.call_count,5)
            self.assertEqual(result['knownLinkResearch']['deferred'],3)
            second=enrich(result,{},d,read_fn=page,propose_fn=model,rules={})
    ...
    def test_one_invalid_quote_does_not_discard_valid_fact(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            result=propose({},packet,d,allow_paid=True,llm_fn=model,isolate_invalid=True)
