...
import { matchItem, sourceKindOf } from "../../app/_lib/display/source.ts";
...
test("processing status filtering composes with existing source and text filters", () => {
  ...
  const source = sourceKindOf(waiting);
  const selected = items.filter(item => categoryOf(item) === "awaiting_body"
    && sourceKindOf(item) === source && matchItem(item, "游戏") && matchDims(item, {}));
