...
function relationshipLabel(company: CompanyProfile, relationship: keyof typeof LABELS): string {
  return relationship === "owned" && (company.entityType === "foundation" || company.entityType === "open_source_organization")
    ? "维护" : LABELS[relationship];
}
export function productText(company: CompanyProfile): string {
  return company.product_names.map(name => {
    ...
    const relations = [...new Set(entries.map(p => relationshipLabel(company, p.relationship)))];
...
export function ProductUpdates({ company }: { company: CompanyProfile }) {
  return <div className="space-y-2">{(company.productUpdates || []).map((p, i) => <div key={`${p.articleId}-${p.name}-${i}`} className="rounded-lg border border-line bg-surface-2 px-3 py-2">
    <span className="font-semibold">{p.name}</span><span className="ml-2 text-[11px] text-brand">{relationshipLabel(company, p.relationship)}</span>
    {p.ownershipEvidence && <a href={p.ownershipEvidence.url} target="_blank" rel="noopener noreferrer" className="ml-2 text-[11px] text-brand hover:underline" title={p.ownershipEvidence.quote}>归属依据</a>}
