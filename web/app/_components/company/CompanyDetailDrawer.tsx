                    ...
                    <div className="mt-1.5 flex flex-wrap gap-1.5">
                      {evidence.map((source, index) => (
                        <a
                          ...
                          title={[source.title, source.value, source.legalEntity, source.countryBasis, source.reason, source.quote, source.checkedAt ? `检查于 ${source.checkedAt}` : null].filter(Boolean).join(" · ")}
                        >
                          {source.verificationStatus === "provisional" ? "暂定 · 待核实" : source.origin === "research" ? "资料核验" : "字段来源"} {index + 1}
                        </a>
                      ))}
                    </div>
                  )}
