...
2026-09-22 起采集改为手动运行，GitHub 定时触发和 Codex 定时跟进均已取消；原北京时间09:30固定采集窗口保持。恢复功能不新增定时任务、不重跑采集、不自动调用模型。
...
2026-09-22故障样本：发现已回收7篇但正文原生进程崩溃，正式批次仅发布AIHOT。解析现已按篇隔离；本地修复探针对7个既有链接读取成功，正文私有保存。它们缺少后续新闻/公司模型结果，不能直接当作上述“零模型恢复”可发布候选，也不能把旧state改成成功。后续只补处理这些证据，避免重复全量Manus发现。
