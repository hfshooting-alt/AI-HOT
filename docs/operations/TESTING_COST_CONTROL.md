...
生产 `call_llm()` 同样显式关闭 DeepSeek V4 思考，按具体阶段读取 `max_output_tokens`。公司抽取默认 6144、融资抽取默认 3072；摘要和轻量探针沿各自较小上限。不能再按旧版统一 1024 上限估算公司 JSON 输出。每日公司背景补全另有独立 5 次请求持久预算，见[补全费用边界](COMPANY_WEB_RESEARCH.md)。

响应明确返回 `finish_reason=length` 时，先保存服务端 token 用量，再记录脱敏错误 `output_limit`；截断正文不进入 JSON 解析，也不写入默认诊断。该类属于单篇输出超限，不累积服务异常熔断，不自动增加输出预算或重试。已授权的独立恢复仍须保留原失败及请求占位；未提供 `finish_reason` 的兼容接口继续沿用原校验。
...
历史：2026-09-10曾设置三次费用止损后阻止排队任务创建；该跨来源规则已于2026-09-14按用户确认撤销。只读GitHub API配置检查工作流不创建Manus任务、不执行模型推理。
