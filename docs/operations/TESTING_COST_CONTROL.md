...
付费问答只验证任务创建、完成和结构化输出，不能证明 20 个账号的网页采集质量。需要真实采集联调时，沿用已授权的小样本范围，事先确定任务数、样本数和观察阈值，默认保留候选产物；当前生产 `run_pipeline.py` 不受上述探针台账限制，不应拿它直接做低成本冒烟。`--no-promote` 仅控制数据发布，不限制费用。
