# 新闻Daily Manus 信源数据契约（MANUS_DATA_CONTRACT）

Manus只依据文章原始发布／发送时间收录，新增点赞、评论、互动、编辑或转载页面的刷新时间均不能使旧文章重新符合窗口。“昨天”等相对文字必须指文章发布，不得引用评论或互动时间。URL路径日期不作为发布时间判定。默认采集窗口现为扫描开始时冻结的最近24小时（北京时间）；旧09:30固定窗口仅显式兼容调用。“昨天”发布的自然日例外保持。
...
新闻来源仅 Manus；完整主链见[统一新闻链路](UNIFIED_NEWS_PIPELINE.md)。所有合规元数据进入全部文章，正文及模型审核通过者进入 Garena投资精选，公司/融资只消费精选。显式空池不回填旧数据；完整正文不公开。下方注明日期的旧 schema/窗口说明只用于兼容和历史追溯。

`AIHOT_ARTICLE` 保留为既有 checkpoint 协议标记，避免使已保存结果失去可解析性；它不表示仍使用 AIHOT 新闻来源。
...
该历史版本窗口为09:30至09:30，当前默认窗口见顶部说明。发现审计新增 `source_status=partial`，必须带说明并保留至少一篇合法文章；`complete` 表示完成窗口扫描，`failed` 表示未获得可用文章且扫描失败。文章仍须通过来源三元组、精确时间、窗口和正文门禁，不能把覆盖不完整当作文章不合法。
...
完整入口采用[统一新闻链路](UNIFIED_NEWS_PIPELINE.md)。该历史版本默认09:30至09:30，`ten-am` 名称保留兼容，当前默认窗口见顶部说明。逐来源发现和正文仍必须通过时间、身份及结构校验。统一 news 阶段允许部分来源失败或合法零篇，并生成新 Manus feed 与共享快照；不套用旧独立 feed 的全来源发布门禁。所有来源不可用或共享模型加工未完成则禁止发布。快照与 feed 的 `collectionStatus` 记录逐来源状态、文章数量和窗口，网页列出缺失来源；禁止从旧正式 feed 补入当前新闻。
...
该历史版本绝对时间使用北京时间前一日09:30至当天09:30的固定窗口，当前默认窗口见顶部说明。原文或对应卡片标注“昨天”时，按采集记录接收时间（北京时间）的前一自然日全天纳入，这是用户授权的日期精度例外，不宣称严格24小时覆盖。published_at为YYYY-MM-DD，publishedPrecision=date；timeEvidence包含originalText和本地observedAt，不生成虚构钟点。历史补跑不能把今天读到的“昨天”解释为历史目标日。
