...
6. `app/_lib/data/api.ts` 读取快照和本站 API；`app/api/` 代理上游；`app/_components/` 渲染页面。
...
| 全部/精选 API 的分页 | `app/_lib/data/items-pool.ts` |
| 上游 ID、来源、分类转换与排序 | `app/_lib/data/news-normalization.ts` |
...
| 配色、布局、公共新闻卡片 | `app/globals.css`、`app/_components/news/ArticleCard.tsx` |
...
- `public/`、`data/`、`archive/` 中有受版本管理的数据产物。结构整理不重新生成这些文件；`build/` 保存 Sites Vite 插件源码。
