...
2026-09-23 增加用户批准的 `direct-only`：配置媒体列表及详情匿名直采，经过同样的来源、原始时间校验后进入本文两池逻辑。标记 `collector=direct_site`，不伪装为Manus。公司、产品与融资仍仅消费精选。见[媒体页面直采](../operations/DIRECT_NEWS_COLLECTION.md)；下方“仅Manus”是新增路线前的历史状态。
