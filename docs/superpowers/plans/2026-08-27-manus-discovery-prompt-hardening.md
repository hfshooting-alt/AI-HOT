...
- Modify: `tests/test_manus_runner.py` — 保留已有 `TestRunnerTimeout`，追加真实 Prompt 渲染测试。
...
预期：工作树已有修改，其中 `scripts/manus_source/runner.py` 有用户改动，`tests/test_manus_runner.py` 当前可能仍为未跟踪文件。记录初始状态；后续仅在指定两个文件内追加本方案内容，不清理、不覆盖其他路径。

### Task 1: 为 Prompt 强化规则建立失败测试

**Files:**

- Modify: `tests/test_manus_runner.py`，在现有 `TestRunnerTimeout` 之后、`if __name__ == "__main__":` 之前追加测试类。
- Test: `tests/test_manus_runner.py`

- [ ] **Step 1: 保留已有超时测试并追加真实 Prompt 渲染夹具**

在 `tests/test_manus_runner.py` 中原样保留 `TestRunnerTimeout`，追加以下完整测试类：
...
- Verify: `tests/test_manus_runner.py`
...
Expected: 本任务新增内容只涉及 `prompts/manus_discovery.md` 和 `tests/test_manus_runner.py`。`TestRunnerTimeout` 保持存在且内容未被删除；初始状态中已有的 `runner.py` 和其他未跟踪文件保持原状。若 `tests/test_manus_runner.py` 仍为未跟踪文件，使用 `Get-Content` 审阅其完整内容，避免 `git diff` 不展示未跟踪文件造成漏检。
...
- Stage: `tests/test_manus_runner.py`
...
- 实施改动仅落在 `prompts/manus_discovery.md` 和 `tests/test_manus_runner.py`；生产脚本、契约、schema、重试、配置和 CLI 均无新增改动。
