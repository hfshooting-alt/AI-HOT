...
**Goal:** Create and publish a concise Chinese `docs/guides/USER_GUIDE.md` for ordinary users of the local AI HOT Next.js dashboard.
...
- Reference: `app/_components/layout/Sidebar.tsx`
- Reference: `app/_components/news/SearchToolbar.tsx`
...
- Reference: `app/_components/funding/FundingTableView.tsx`

- [ ] **Step 1: Add the title, quick access, and reading path**

Create `docs/guides/USER_GUIDE.md` with the title `# AI HOT 使用说明`, the localhost URL, a one-paragraph product description, and a short “第一次使用” sequence: start or open the local site, choose a view, use filters, then open the original article. State that the old CloudBase page is retired.
...
**Files:**
- Review: `docs/guides/USER_GUIDE.md`
- Test: `tests/`
...
```powershell
rg -n "精选|全部 AI 动态|热点榜|AI 日报|设置|融资动态|一手信源|公众号|USER_GUIDE|localhost:3000|\.env" docs/guides/USER_GUIDE.md
```
...
```powershell
rg -n "T[B]D|TO[D]O|不[是].*而[是]|sk-[A-Za-z0-9]{12,}|tvly-[A-Za-z0-9]{12,}" docs/guides/USER_GUIDE.md
```
...
```powershell
node tests/frontend/rendered-html.test.mjs
node tests/frontend/source-kind.test.mjs
node tests/settings/test_settings_server.mjs
```
...
**Files:**
- Commit: `docs/guides/USER_GUIDE.md`
...
```powershell
git add -- docs/guides/USER_GUIDE.md
